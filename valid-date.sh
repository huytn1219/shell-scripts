#!/bin/bash
# valid-date--Validate a date, taking into account leap year rules
...
isLeapYear()
{
    ...
    #   1 otherwise.
    # The fomula for checking whether a year is a leap year is:
    ...
    #   4. All other years divisible y 4 are leap years.
    year=$1
    if [ "$((year % 4))" -ne 0 ] ; then
...
# Now that we have normalized date, let's check whether the
